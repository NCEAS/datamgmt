...
factors1 <- data.frame(attributeName = 'Levs', code = names(Levs), definition = unname(Levs))
factors1
...
#Problem 1: Currently only works for nominal/text data.
#Problem 2: Fails if any arguments existing within a certain measurementScale are not specified
...
edit_attribute <- function(eml, dataTable, attribute, attributeName=NA, attributeDefinition=NA, measurementScale=NA, unit=NA, numberType=NA,
                           definition=NA, formatString=NA, missingValueCode=NA, missingValueCodeExplanation=NA){
