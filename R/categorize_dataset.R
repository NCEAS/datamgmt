...
#' @examples #categorize_dataset("doi:10.18739/A2QJ77Z09", c("biology", "oceanography"), "your name")
#' @author Jasmine Lai
#' @export
categorize_dataset <- function(doi, themes, coder, test = F) {
    ...
    # if(length(problem_themes) == 1){
    #     print(paste("Please check if this is one of the accepted categories:", themes[[problem_themes]]))
    #     stop()
    #     } else if (length(problem_themes) > 1){
    #     purrr::map(problem_themes, ~print(paste("Please check if this is one of the accepted categories:", themes[[.x]])))
    #     stop()
    # }
