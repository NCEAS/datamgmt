...
### Changing DataOne/LTER Member Nodes 

We want to download this package from LTER: https://search.dataone.org/#view/https://pasta.lternet.edu/package/metadata/eml/knb-lter-ntl/276/13.  We first need to tell R that we're changing member nodes.  The fastest way of doing this is to reset your `mn` with the `guess_member_node` function.

*Note:* 
```{r}
#' The following two calls return the same member node (mn)
#' The first uses the metadata identifier and the second uses the resource map identifier:
